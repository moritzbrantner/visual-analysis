...
use video_analysis_core::{Observation, ObservationKind, Result, VideoAnalyzer, VideoFrame};

/// Adapts an image-analysis OCR backend to the shared video-analysis pipeline.
///
/// The adapter owns no OCR implementation. It converts a decoded video frame into
/// the image-analysis view, delegates recognition to the supplied `OcrBackend`,
/// and projects structured OCR output into timestampable video observations.
...
/// is intentionally left unset here so `VideoAnalysisPipeline` can stamp the
/// source frame consistently for every analyzer.
pub fn ocr_document_observations(analyzer: &str, document: &OcrDocument) -> Vec<Observation> {
    let mut observations = Vec::new();

    for (block_index, block) in document.blocks.iter().enumerate() {
        if block.lines.is_empty() {
            if let Some(observation) = text_observation(
                analyzer,
                block.text.trim(),
                block.region,
                block.confidence,
