...
#[derive(Debug, Parser)]
#[command(
    name = "vision-core-cli",
    version,
    about = "Thin CLI adapter for vision-core"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "vision-core",
            &vision_core_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "vision-core command schema",
            &vision_core_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&vision_core_cli::package_surface().operations)?;
            print_payload(json, "vision-core operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response =
                vision_core_cli::run_operation(&operation, input).map_err(std::io::Error::other)?;
